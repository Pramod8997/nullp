...
Repo: /home/pramodsb/Downloads/mjr | Baseline: 433/433 Tests Passing (100%).
...
### 2. EXACT API CONTRACTS (ANTI-HALLUCINATION)
Never invent APIs. Refer to `claude_context/ARCHITECTURE_AND_APIS.md`:
...
- Physical Realities: 3.3V GPIO 18 requires 2N7000 MOSFET level shifter to 5V relay IN; RC snubber (100Ω + 0.1µF X2) across relay COM-NO; HLK-5M05 PSU + 1000µF low-ESR bulk capacitor.

### 4. WORKFLOW & VERIFICATION COMMANDS
1. Query Architecture: `graphify query "<question>"`
2. Run Full Regression: `source .venv/bin/activate && python -m pytest tests/ -q`
3. Run Core Suites: `python -m pytest tests/test_hil_uart_corruption.py tests/test_relay_safety_boot_brownout.py tests/test_ml_nilm_math_stress.py tests/test_security_penetration.py tests/test_chaos_engineering.py -v --tb=short`
4. Run Physical Harness: `python scripts/real_world_physical_stress.py && python scripts/hil_hardware_test.py`
5. Keep Graph Synced: `graphify update .` (AST-only, run after any file edit)

Context References (Read only when needed):
- Architecture/APIs: `claude_context/ARCHITECTURE_AND_APIS.md`
- PRD: `claude_context/PRD.md` | Tech Review: `claude_context/TECHNICAL_REVIEW.md`
- Hardware Guide: `claude_context/HARDWARE_DEPLOYMENT_GUIDE.md` | Physical Plan: `claude_context/REAL_WORLD_TESTING_PLAN.md`
```
