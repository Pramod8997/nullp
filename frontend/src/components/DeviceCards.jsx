    ...
    <div>
      ...
      <div className="device-cards">
        {deviceIds.map(id => {
          ...
          const cardClass = `device-card ${isOn ? 'state-on' : 'state-off'} ${isGlow ? 'glow' : ''}`.trim();

          return (
            <div key={id} data-testid={`device-card-${id}`} className={cardClass}>
              <div data-testid="device-card" className={cardClass}>
                ...
                <div className="device-card__power">
                  {power === 0 || !isOn ? '0W' : `${power.toFixed(1)} W`}
                </div>
